...
import sys
print(sys.path)
print("configs import:", __import__("configs"))
print("datasets import:", __import__("datasets"))
    ...
    for input_batch in tqdm(dataloader):
        ...
        for i in range(input_batch.shape[0]):
            ...
            if opts.save_weight_deltas:
                ...
                delta = result_deltas[i][-1]
                delta_path = os.path.join(weight_deltas_dir, os.path.basename(im_path).split('.')[0] + ".npy")
                delta_obj = np.array(delta, dtype=object)
                np.save(delta_path, delta_obj)
