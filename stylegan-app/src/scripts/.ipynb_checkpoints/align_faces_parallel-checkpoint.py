...
def align_face(filepath, predictor):
    ...
    x /= np.hypot(*x)
    # Tweak for tighter crop:
    x *= max(np.hypot(*eye_to_eye) * crop_eye_factor, np.hypot(*eye_to_mouth) * crop_mouth_factor)
    y = np.flipud(x) * [-1, 1]
    ...
    homography_path = filepath + '_homography.json'
